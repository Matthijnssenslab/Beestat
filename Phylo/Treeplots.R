setwd("C:/Users/wardd/OneDrive/Papers/qPCR/Phylo")
library(ggtree)
library(phangorn)
library(ggplot2)
library(gridExtra)
#Donkeybridge: + geom_text(aes(label=node)) is for node number annotation.




#Orthomyxo
orthotree <- read.tree("Orthomyxo_blastquer_AL_TRIM.faa.treefile")
#midpoint root
orthotree <- phangorn::midpoint(orthotree)
orthotree <- reorder(orthotree)
cls_thog <- list(
  'NODE' = c('Apis_Orthomyxovirus1'),
...
orthodraw <- ggtree(orthotree, aes(color=group)) + 
  ...
  geom_cladelabel(node=28, label="Thogoto-like viruses", align=T, geom='label', col='#1f77b4', offset=0.8) +
  geom_cladelabel(node=21, label="Influenza", align=T, geom='label', col='#ff7f0e', offset=0.8) +
  xlim(0, 5) +
...
#thikatree
thikatree <- read.tree("Thika_blastquer50_AL_TRIM.faa.treefile")
#midpoint root
thikatree <- phangorn::midpoint(thikatree)
thikatree <- reorder(thikatree)

cls_thika <- list(
  'NODE' = c("Apis_thika-like_virus"),
...
cols_thika <- c("Unclassified" = "#7f7f7f","crawfish virus" = "#1f77b4", "Acyrthosiphon pisum virus" = "#ff7f0e","NODE" = "#d62728", "Old port virus" = "#2ca02c","Thika"="#9467bd")

thikatree <- groupOTU(thikatree, cls_thika)

thikadraw <- ggtree(thikatree, aes(color=group)) + 
  geom_nodepoint(aes(subset = as.numeric(label) > 70), color='black',size=1) +
  geom_tiplab(align=TRUE, size=3) +
  scale_colour_manual(values = cols_thika) + 
  geom_treescale(offset=-1.5) +
  geom_cladelabel(node=86, label="Thika virus", align=T, geom='label', col='#9467bd', offset=0.55) +
  geom_cladelabel(node=58, label="Acyrthosiphon pisum virus", align=T, geom='label', col='#ff7f0e', offset=0.50) +
  geom_cladelabel(node=55, label="Changjiang crawfish virus", align=T, geom='label', col='#1f77b4', offset=0.55) +
  geom_cladelabel(node=78, label="Old port virus", align=T, geom='label', col='#2ca02c', offset=0.45) +
  xlim(0, 4) +
  ggtitle("Apis Thika-like virus") + 
  theme(plot.title = element_text(size=22)) +
  theme(plot.title = element_text(hjust = 0.5))
thikadraw
...
#Unc 
unctree <- read.tree("Unc_blastquer_AL_TRIM.faa.treefile")
#midpoint root
unctree <- phangorn::midpoint(unctree)
unctree <- reorder(unctree)
cls_unc <- list(
  "NODE" = c("Apis_Uncultured_virus"),
...
cols_unc <-c("Animal" = "#1f77b4", "Partiti" = "#ff7f0e","NODE" = "#d62728","RNAvirosph_insect"="#9467bd")

unctree <- groupOTU(unctree, cls_unc)

uncdraw <- ggtree(unctree, aes(color=group)) + 
  geom_nodepoint(aes(subset = as.numeric(label) > 70), color='black',size=1) +
  geom_tiplab(align=TRUE, show.legend=FALSE, size=3)+
  scale_colour_manual(values = cols_unc) + 
  geom_treescale(offset=-1.5) +
  xlim(0, 3) +
  theme(legend.position="right") +
  ggtitle("Apis Uncultured virus") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(size=22)) +
...
#BMLV
bmlvtree <- read.tree("BMLV_blastquer50_AL_TRIM.faa.treefile")
#midpoint root
...
bmlvdraw <- ggtree(bmlvtree, aes(color=group)) + 
  ...
  geom_cladelabel(node=152, label="Marafivirus", align=T, geom='label', col='#ff7f0e', offset=0.35) +
  geom_cladelabel(node=105, label="Tymovirus", align=T, geom='label', col='#1f77b4', offset=0.35) +
  geom_cladelabel(node=190, label="Marafivirus", align=T, geom='label', col='#ff7f0e', offset=0.35) +
  geom_cladelabel(node=191, label="Maculavirus", align=T, geom='label', col='#9467bd', offset=0.35) +
  geom_cladelabel(node=200, label="Maculavirus", align=T, geom='label', col='#9467bd', offset=0.35) +
  geom_treescale(offset=-1.5) +
...
#AMFV
amfvtree <- read.tree("AMFV_blastquer_viruses_AL_TRIM.faa.treefile")
#midpoint root
...
amfvdraw <- ggtree(amfvtree, aes(color=group)) + 
  ...
  geom_treescale(offset=-1.5) +
  geom_cladelabel(node=106, label="Poxviridae", align=T, geom='label', col='#1f77b4', offset=0.24) +
  geom_cladelabel(node=182, label="Baculoviridae", align=T, geom='label', col='#9467bd', offset=0.24) +
  geom_cladelabel(node=197, label="Phycodnaviridae", align=T, geom='label', col='#ff7f0e', offset=0.19) +
...
#vdv
vdvtree <- read.tree("vdv_phylorefseq_AL_TRIM.faa.treefile")
#midpoint root
...
testgrid <- grid.arrange(amfvdraw,bmlvdraw,uncdraw,vdvdraw, thikadraw ,orthodraw,layout_matrix =lay)

ggsave("treeplots.pdf",testgrid, device = "pdf", height=20,width=23.5, limitsize=FALSE)
